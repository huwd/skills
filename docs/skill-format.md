...
This repository's `skills/` directory is the canonical home for reusable agent skills that should work across Codex, Claude Code, and OpenCode where possible.
...
Interim approach until skills are installed with agent-manager (see `docs/plan.md`, phase 3).
...
```text
~/Projects/skills/skills/<skill-name>
```
...
ln -sfn ~/Projects/skills/skills/<skill-name> ~/.agents/skills/<skill-name>

# Claude Code
ln -sfn ~/Projects/skills/skills/<skill-name> ~/.claude/skills/<skill-name>

# Codex
ln -sfn ~/Projects/skills/skills/<skill-name> ~/.codex/skills/<skill-name>

# OpenCode native location, optional if ~/.agents/skills is already used
ln -sfn ~/Projects/skills/skills/<skill-name> ~/.config/opencode/skills/<skill-name>
```
...
Keep portable prompts or procedures here as skills. Keep harness-specific agent definitions in a separate `agents/` tree if and when one is needed.
